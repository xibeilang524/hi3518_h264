...
功能:根据图像制式，图片尺寸，图片格式及对齐方式计算一个视频块大小
参数:
	enNorm:		in,图像制式
	enPicSize:	in,图片尺寸
	enPixFmt:	in,图片格式
	u32AlignWidth:in,对齐方式
返回值:视频块大小
******************************************************************************/
//SAMPLE_COMM_SYS_CalcPicVbBlkSize(VIDEO_ENCODING_MODE_NTSC,PIC_QVGA, SAMPLE_PIXEL_FORMAT, SAMPLE_SYS_ALIGN_WIDTH);
...
//u32VbSize=122880
    ...
    VB_PIC_HEADER_SIZE(stSize.u32Width, stSize.u32Height, enPixFmt, u32HeaderSize);//u32HeaderSize=5760
    u32VbSize += u32HeaderSize;

    return u32VbSize;//u32VbSize=122880+5760=128640
}
...
HI_S32 SAMPLE_COMM_SYS_Init(VB_CONF_S *pstVbConf)
{
    ...
    HI_MPI_VB_Exit();//去初始化 MPP 视频缓存池,必须先去初始化MPP 系统
    ...
    s32Ret = HI_MPI_VB_SetConf(pstVbConf);//设置 MPP 视频缓存池属
    if (HI_SUCCESS != s32Ret)
    {
