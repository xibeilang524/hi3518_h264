...
/*
    stViConfig.enViMode   = SENSOR_TYPE;//SENSOR_TYPE=APTINA_AR0130_DC_720P_30FPS = 0,
...
HI_S32 SAMPLE_COMM_VI_StartVi(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    ...
    enViMode = pstViConfig->enViMode;//=0
    if(!IsSensorInput(enViMode))//IsSensorInput(enViMode)==1
    {
        s32Ret = SAMPLE_COMM_VI_StartBT656(pstViConfig);
    }
    else
    {
        s32Ret = SAMPLE_COMM_VI_StartIspAndVi(pstViConfig);//执行这里
    }
...
根据传感器确定图片大小
参数:	IO，传入默认1080，输出由传感器决定，这里是720P
返回:	成功返回0，失败返回-1
******************************************************************************/
